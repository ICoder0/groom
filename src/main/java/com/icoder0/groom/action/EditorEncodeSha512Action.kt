 ...
 * @since 2021/3/13
 */
class EditorEncodeSha512Action : AnAction() {
    /**
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        val dialog = EncodeSha512Dialog()
        var original = document.text;
        if (dialog.showAndGet()) {
            val charset = Charset.forName(dialog.getCharset())
            original = Hex.toHexString(MessageDigest.getInstance("SHA-512").digest(document.text.toByteArray(charset)))
            if (dialog.isUpper){
    ...
    class EncodeSha512Dialog : DialogWrapper(true) {
        var isUpper = true
