 ...
 * @since 2021/3/13
 */
class EditorEncodeSha1Action : AnAction() {
    /**
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        val dialog = EncodeSha1Dialog()
        var original = document.text;
        if (dialog.showAndGet()) {
            val charset = Charset.forName(dialog.getCharset())
            original = Hex.toHexString(MessageDigest.getInstance("SHA-1").digest(document.text.toByteArray(charset)))
            if (dialog.isUpper){
    ...
    class EncodeSha1Dialog : DialogWrapper(true) {
        var isUpper = true
